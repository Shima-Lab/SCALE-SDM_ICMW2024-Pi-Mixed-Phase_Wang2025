...
!! @li      2021-07-03 (S.Shima) [fix] index range of sdasl_fm(n,s)
!!
!<
!-------------------------------------------------------------------------------
module m_sdm_memmgr
...
!       allocate(sdice_fm%t(1:1))
    ...
    if( sdm_cold ) then
       allocate(sd_icetmp1%re(1:sdnum_s2c))
       allocate(sd_icetmp1%rp(1:sdnum_s2c))
       allocate(sd_icetmp1%rho(1:sdnum_s2c))
       allocate(sd_icetmp1%tf(1:sdnum_s2c))
       allocate(sd_icetmp1%mrime(1:sdnum_s2c))
       allocate(sd_icetmp1%nmono(1:sdnum_s2c))
    else
       allocate(sd_icetmp1%re(1:1))
       allocate(sd_icetmp1%rp(1:1))
       allocate(sd_icetmp1%rho(1:1))
       allocate(sd_icetmp1%tf(1:1))
       allocate(sd_icetmp1%mrime(1:1))
       allocate(sd_icetmp1%nmono(1:1))
    end if
