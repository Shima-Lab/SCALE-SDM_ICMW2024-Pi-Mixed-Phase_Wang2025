...
!!$  real(RP), allocatable, save :: rhoa_sdm(:,:,:) ! density of aerosol
  ...
  real(RP), allocatable, target, save :: sd_dtmp4(:)
  real(RP), allocatable, target, save :: sd_dtmp5(:)
...
!!$     ! temperature of ice crystal
  ...
  type(sdicedef), target :: sd_icetmp1
...
!!  real(RP), parameter :: rho_amsul = 1.769E+6_RP, rho_nacl = 2.165E+6_RP
  ...
  ! parameter for 2mode log-nomiral distribution for DYCOMS2(RF02) : Ackerman et al. (2009)
  real(RP), parameter :: n1_amsul_dycoms   = 125.e6_RP     !! [1/m3]
  real(RP), parameter :: n2_amsul_dycoms   = 65.e6_RP
  real(RP), parameter :: rb1_amsul_dycoms  =  1.14e-08_RP  !! [m]
  real(RP), parameter :: rb2_amsul_dycoms  =  7.95e-08_RP
  real(RP), parameter :: sgm1_amsul_dycoms =  1.20e0_RP    !! [-]
  ...
  integer, save :: sdm_aslset   = 1           ! Conrol flag to set the species and way of chemical material as water-soluble aerosol
  ...
  real(RP), save :: sdm_aslfm_starttime =  60.0_RP ! time [sec] of start aerosol nucleation
  real(RP), save :: sdm_icefm_starttime = 120.0_RP ! time [sec] of start ice injection
  ...
  real(RP), save :: sdm_icefmrate = 0.0_RP ! Ice formation rate [1/(m^3 s)]
