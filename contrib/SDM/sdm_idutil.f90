...
!! @li      2021-12-24 (S.Takahashi) [add] 'valid' options to sdm_copy_selected_sd 
!!
!<
!-------------------------------------------------------------------------------
module m_sdm_idutil
...
contains
  ...
  subroutine sdm_copy_selected_sd(sd_num,    sd_numasl,    sd_n,    sd_x,    sd_y,    sd_ri,    sd_rj,    sd_rk,     &
    ...
    else if (sdtype == 'large_drop') then
       sd_thld_radi = 3.5e-6_RP ! threshold radius [m] (following ICMW2024 Pi case)
       do n=1,sd_num
    ...
    else if (sdtype == 'small_drop') then
       sd_thld_radi = 3.5e-6_RP ! threshold radius [m] (following ICMW2024 Pi case)
       do n=1,sd_num
          if( sd_rk(n)<VALID2INVALID ) cycle
    ...
    else if (sdtype == 'ice') then
       do n=1,sd_num
          if( sd_rk(n)<VALID2INVALID ) cycle

          if( sd_liqice(n) == STAT_ICE ) then
            cnt = cnt + 1
            ilist(cnt) = n
          end if
       end do
