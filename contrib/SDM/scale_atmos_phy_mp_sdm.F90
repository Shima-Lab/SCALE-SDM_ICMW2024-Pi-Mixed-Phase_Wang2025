...
!! @li      2021-06-07 (S.Shima) [mod] solid slip wall for the side boundaries
!! @li      2021-07-02 (S.Shima) [add] aslset=66 for ICMW PI
!! @li      2021-07-03 (S.Shima) [add] aslform for aslset=66
!! @li      2021-07-10 (S.Shima) [fix] overflow of aslform
!! @li      2022-12-24 (S.Takahashi) [mod] change sdtype parameter from "activated" to "valid" 
!<  
!-------------------------------------------------------------------------------
...
!    real(RP) :: delta1, delta2 !, sdn_tmp
    ...
    !--- flags for the boundary condition, but no other option other than 1 for now
    wbc=1
    ebc=1
    sbc=1
    nbc=1
...
!!$    end if
    ...
    !! check sdm_aslfmdt
    if( sdm_aslfmdt>0.0_RP ) then
        if(mod(int(1.E+6_RP*(sdm_aslfmdt+1.E-7_RP)),int(1.E+6_RP*(dt+1.E-7_RP))) /= 0) then
           if( IO_L ) write(IO_FID_LOG,*) 'ERROR: sdm_aslfmdt should be multiple of TIME_DTSEC_ATMOS_PHY_MP'
           call PRC_MPIstop
        end if
     end if
...
!!$    dz_sdm(1:KA) = FDZ(1:KA)
    ...
    call sdm_numset(              &
      ...
      minzph, sdininum_s2c,       &
      sdfmnum_s2c, sdfmnumreal_s2c, sdnum_s2c,     &
  ...
  subroutine ATMOS_PHY_MP_sdm( &
    ...
    use m_sdm_sd2fluid, only: &
         sdm_sd2qcqr,sdm_sd2qiqsqg,sdm_sd2rhosd,sdm_sd2rhodropmom,sdm_sd2rhoicemom
    ...
    real(RP) :: rhoicemom0(KA,IA,JA) ! density of the 0th ice moment
    real(RP) :: rhoicemom1(KA,IA,JA) ! density of the 1st ice moment
    real(RP) :: rhoicemom2(KA,IA,JA) ! density of the 2nd ice moment
    real(RP) :: rhoicemom3(KA,IA,JA) ! density of the 3rd ice moment
    ...
    if( ((mod(sdm_dmpvar,1000))/100>=1) .and. sdm_dmpitvl>0.0_RP .and. &
       ...
       !     &                    t_scale,sd_itmp1,sdtype='activated') ! options: 'all', 'large', 'activated' 
            &                    t_scale,sd_itmp1,sdtype='valid') ! options: 'all', 'large', activated, 'valid' 
...
!     end if
     ...
     if( sdm_aslfmdt>0.0_RP                             .and. &
         (int(1.E+4_RP*(TIME_NOWSEC+1.E-5_RP),kind=8) >=             & 
          int(1.E+4_RP*(sdm_aslfm_starttime+1.E-5_RP),kind=8)) .and. &  
         (mod(int(1.E+4_RP*(TIME_NOWSEC+1.E-5_RP),kind=8),           &
             int(1.E+4_RP*(sdm_aslfmdt+1.E-5_RP),kind=8)) == 0)      &
       ) then

        if( IO_L ) write(IO_FID_LOG,*) '****** SDM: NaCl injection'

        call sdm_aslform1(DENS,RHOT,QTRC,                             &   
                         ...
                         sdv_s2c,sdvz_s2c,sdr_s2c,sdasl_s2c,sdice_s2c,         &
                         sdfmnum_s2c,sdfmnumreal_s2c,sdn_fm,sdliqice_fm,sdx_fm,sdy_fm,sdz_fm,    &
                         sdri_fm,sdrj_fm,sdrk_fm,sdvz_fm,sdr_fm,sdasl_fm,sdice_fm,            &
                         rng_s2c,               &
                         sd_itmp1,sd_itmp2,sd_itmp3)

     end if

     ! Ice particle injection
     if( sdm_aslfmdt>0.0_RP                             .and. &
         (int(1.E+4_RP*(TIME_NOWSEC+1.E-5_RP),kind=8) >=             & 
          int(1.E+4_RP*(sdm_icefm_starttime+1.E-5_RP),kind=8)) .and. &  
         (mod(int(1.E+4_RP*(TIME_NOWSEC+1.E-5_RP),kind=8),           &
             int(1.E+4_RP*(sdm_aslfmdt+1.E-5_RP),kind=8)) == 0)      &
       ) then

        if( IO_L ) write(IO_FID_LOG,*) '****** SDM: ice injection'

        call sdm_aslform2(DENS,RHOT,QTRC,                             &   
                         sdm_calvar,sdm_aslset,                      &
                         sdm_aslfmsdnc,sdm_sdnmlvol,                 &
                         sdm_zupper,sdm_zlower,sdm_dtcmph,           &
                         sdnum_s2c,sdnumasl_s2c,sdn_s2c,sdliqice_s2c,sdx_s2c,     &
...
!!$    call HIST_in( rhoa_sdm(:,:,:), 'RAERO', 'aerosol mass conc.', 'kg/m3', dt)
    ...
    if( do_puthist ) then
       ...
       call sdm_copy_selected_sd(sdnum_s2c,sdnumasl_s2c,sdn_s2c,sdx_s2c,sdy_s2c,sdri_s2c,sdrj_s2c,sdrk_s2c, &
            ...
            &                    sdliqice_tmp,sdasl_tmp,sdr_tmp,sdice_tmp,                                  &
       !     &                    t_scale,sd_itmp1,sdtype='activated') ! options: 'all', 'large', 'activated'
            &                    t_scale,sd_itmp1,sdtype='large_drop') ! options: 'all', 'large', 'activated', 'valid'
    ...
    !!!!!!!!!!!!!!!!!!!!!
    !!!! History output of the density of aerosol moments.
    !!!! Data is updated only at the time of output
    do_puthist = .false.
    ! Check whether the item has been already registered
    call HIST_reg( histitemid, 'NASL', 'aerosol number density', 'num/m3', 3)
    ! Check whether it is time to input the item
    do_puthist_0 = .false.
    call HIST_query( histitemid,  & ! [IN]
                     do_puthist_0   ) ! [OUT]
    ...
    ! Check whether the item has been already registered
    call HIST_reg( histitemid, 'AMOM1', 'density of the 1st aerosol moment', 'm/m3', 3)
    ! Check whether it is time to input the item
    do_puthist_1 = .false.
    call HIST_query( histitemid,  & ! [IN]
                     do_puthist_1   ) ! [OUT]
    ...
    ! Check whether the item has been already registered
    call HIST_reg( histitemid, 'AMOM2', 'density of the 2nd aerosol moment', 'm2/m3', 3)
    ! Check whether it is time to input the item
    do_puthist_2 = .false.
    call HIST_query( histitemid,  & ! [IN]
                     do_puthist_2   ) ! [OUT]
    ...
    ! Check whether the item has been already registered
    call HIST_reg( histitemid, 'AMOM3', 'density of the 3rd aerosol moment', 'm3/m3', 3)
    ! Check whether it is time to input the item
    do_puthist_3 = .false.
    call HIST_query( histitemid,  & ! [IN]
                     do_puthist_3   ) ! [OUT]
    ...
    if( do_puthist ) then
       ...
       call sdm_copy_selected_sd(sdnum_s2c,sdnumasl_s2c,sdn_s2c,sdx_s2c,sdy_s2c,sdri_s2c,sdrj_s2c,sdrk_s2c, &
            ...
            &                    sdliqice_tmp,sdasl_tmp,sdr_tmp,sdice_tmp,                                  &
       !     &                    t_scale,sd_itmp1,sdtype='activated') ! options: 'all', 'large', 'activated'
            &                    t_scale,sd_itmp1,sdtype='small_drop') ! options: 'all', 'large', 'activated', 'valid'
       ...
       if( do_puthist_0 )then
          ...
          call sdm_sd2rhodropmom(order_n,zph_crs,rhodropmom0,sdnum_tmp,sdn_tmp,sdliqice_tmp, &
               & sdx_tmp,sdy_tmp,sdr_tmp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               & sd_itmp1)
          call HIST_in( rhodropmom0(:,:,:), 'NASL', 'aerosol number density', 'num/m3')
       end if
       ...
       if( do_puthist_1 )then
          ...
          call sdm_sd2rhodropmom(order_n,zph_crs,rhodropmom1,sdnum_tmp,sdn_tmp,sdliqice_tmp, &
               & sdx_tmp,sdy_tmp,sdr_tmp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               & sd_itmp1)
          call HIST_in( rhodropmom1(:,:,:), 'AMOM1', 'density of the 1st aerosol moment', 'm/m3')
       end if
       ...
       if( do_puthist_2 )then
          ...
          call sdm_sd2rhodropmom(order_n,zph_crs,rhodropmom2,sdnum_tmp,sdn_tmp,sdliqice_tmp, &
               & sdx_tmp,sdy_tmp,sdr_tmp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               & sd_itmp1)
          call HIST_in( rhodropmom2(:,:,:), 'AMOM2', 'density of the 2nd aerosol moment', 'm/m3')
       end if
       ...
       if( do_puthist_3 )then
          ...
          call sdm_sd2rhodropmom(order_n,zph_crs,rhodropmom3,sdnum_tmp,sdn_tmp,sdliqice_tmp, &
               & sdx_tmp,sdy_tmp,sdr_tmp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               & sd_itmp1)
          call HIST_in( rhodropmom3(:,:,:), 'AMOM3', 'density of the 3rd aerosol moment', 'm/m3')
       end if
    ...
    !!!!!!!!!!!!!!!!!!!!!
    !!!! History output of the density of ice moments.
    !!!! Data is updated only at the time of output
    do_puthist = .false.
    ! Check whether the item has been already registered
    call HIST_reg( histitemid, 'NI', 'ice number density', 'num/m3', 3)
    ! Check whether it is time to input the item
    do_puthist_0 = .false.
    call HIST_query( histitemid,  & ! [IN]
                     do_puthist_0   ) ! [OUT]
    ...
    ! Check whether the item has been already registered
    call HIST_reg( histitemid, 'IMOM1', 'density of the 1st ice moment', 'm/m3', 3)
    ! Check whether it is time to input the item
    do_puthist_1 = .false.
    call HIST_query( histitemid,  & ! [IN]
                     do_puthist_1   ) ! [OUT]
    ...
    ! Check whether the item has been already registered
    call HIST_reg( histitemid, 'IMOM2', 'density of the 2nd ice moment', 'm2/m3', 3)
    ! Check whether it is time to input the item
    do_puthist_2 = .false.
    call HIST_query( histitemid,  & ! [IN]
                     do_puthist_2   ) ! [OUT]
    ...
    ! Check whether the item has been already registered
    call HIST_reg( histitemid, 'IMOM3', 'density of the 3rd ice moment', 'm3/m3', 3)
    ! Check whether it is time to input the item
    do_puthist_3 = .false.
    call HIST_query( histitemid,  & ! [IN]
                     do_puthist_3   ) ! [OUT]
    ...
    if( do_puthist ) then
       ...
       call sdm_copy_selected_sd(sdnum_s2c,sdnumasl_s2c,sdn_s2c,sdx_s2c,sdy_s2c,sdri_s2c,sdrj_s2c,sdrk_s2c, &
            ...
            &                    sdliqice_tmp,sdasl_tmp,sdr_tmp,sdice_tmp,                                  &
       !     &                    t_scale,sd_itmp1,sdtype='activated') ! options: 'all', 'large', 'activated'
            &                    t_scale,sd_itmp1,sdtype='ice') ! options: 'all', 'large', 'activated', 'valid'

       if( do_puthist_0 )then
          order_n = 0
          call sdm_sd2rhoicemom(order_n,zph_crs,rhoicemom0,sdnum_tmp,sdn_tmp,sdliqice_tmp, &
               & sdx_tmp,sdy_tmp,sdice_tmp%re,sdice_tmp%rp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               !& sdx_tmp,sdy_tmp,sdr_tmp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               & sd_itmp1)
          call HIST_in( rhoicemom0(:,:,:), 'NI', 'ice number density', 'num/m3')
       end if

       if( do_puthist_1 )then
          order_n = 1
          call sdm_sd2rhoicemom(order_n,zph_crs,rhoicemom1,sdnum_tmp,sdn_tmp,sdliqice_tmp, &
               & sdx_tmp,sdy_tmp,sdice_tmp%re,sdice_tmp%rp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               !& sdx_tmp,sdy_tmp,sdr_tmp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               & sd_itmp1)
          call HIST_in( rhoicemom1(:,:,:), 'IMOM1', 'density of the 1st ice moment', 'm/m3')
       end if

       if( do_puthist_2 )then
          order_n = 2
          call sdm_sd2rhoicemom(order_n,zph_crs,rhoicemom2,sdnum_tmp,sdn_tmp,sdliqice_tmp, &
               & sdx_tmp,sdy_tmp,sdice_tmp%re,sdice_tmp%rp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               !& sdx_tmp,sdy_tmp,sdr_tmp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               & sd_itmp1)
          call HIST_in( rhoicemom2(:,:,:), 'IMOM2', 'density of the 2nd ice moment', 'm/m3')
       end if

       if( do_puthist_3 )then
          order_n = 3
          call sdm_sd2rhoicemom(order_n,zph_crs,rhoicemom3,sdnum_tmp,sdn_tmp,sdliqice_tmp, &
               & sdx_tmp,sdy_tmp,sdice_tmp%re,sdice_tmp%rp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               !& sdx_tmp,sdy_tmp,sdr_tmp,sdri_tmp,sdrj_tmp,sdrk_tmp,sdrkl_s2c,sdrku_s2c,    &
               & sd_itmp1)
          call HIST_in( rhoicemom3(:,:,:), 'IMOM3', 'density of the 3rd ice moment', 'm/m3')
       end if
...
!      call rng_load_state( rng_s2c, trim(RANDOM_IN_BASENAME), fid_random_i )
         ...
         do n=1,sdnum_s2c
            ...
            if( k==i ) then
               ...
               if( abs(mod(sdm_aslset,10))==1 .or.                      &
                  ...
                  !! muliplicity
                  sdn_tmp = n0/(delta1*delta2)
               else if( sdm_aslset==66 ) then
                  !### init+rest : NaCl [g] (ICMW_PI) ###!
                  dry_r = 0.125e-6_RP ! [m]
                  sdasl_s2c(n,k) = F_THRD * ONE_PI                      &
                                * (dry_r*dry_r*dry_r) * rho_nacl
      ...
      if( sdm_aslset==66 ) then
         !### ICMW_PI, no aeorosl initially ###!
         do n=1,sdnum_s2c
            sdz_s2c(n) = INVALID
         end do
      else
...
!  end subroutine sdm_sd2momnt
  ...
  !----------------------------------------------------------------------------
  subroutine sdm_aslform1(DENS,RHOT,QTRC,                          &   
                         sdm_calvar,sdm_aslset,                   &
                         sdm_aslfmsdnc,sdm_sdnmlvol,              &
                         sdm_zupper,sdm_zlower,sdm_dtcmph,        &
                         sd_num,sd_numasl,sd_n,sd_liqice,sd_x,sd_y,sd_z,    &
                         sd_ri,sd_rj,sd_rk,sd_u,sd_v,sd_vz,sd_r,sd_asl,sdi,       &
                         sd_fmnum,sd_fmnumreal,sd_fmn,sd_fmliqice,sd_fmx,sd_fmy,sd_fmz,    &
                         sd_fmri,sd_fmrj,sd_fmrk,sd_fmvz,sd_fmr,sd_fmasl,sd_fmi,  &
                         sd_rng,               &
                         sd_itmp1,sd_itmp2,sd_itmp3)
    ...
    real(RP), intent(in) :: QTRC(KA,IA,JA,QAD)    !! Ratio of mass of tracer to total mass[kg/kg]
      ...
      real(RP), intent(in) :: sd_fmnumreal ! sd_fmnum=int(sd_fmnumreal)+1
      ! Input and output variables
      integer(DP), intent(inout) :: sd_n(1:sd_num)   ! multiplicity of super-droplets
      integer(i2), intent(inout) :: sd_liqice(1:sd_num)
                       ! status of super-droplets (liquid/ice)
      ...
      integer(i2), intent(inout) :: sd_fmliqice(1:sd_fmnum)
                       ! status of super-droplets (liquid/ice)
                       ! 01 = all liquid, 10 = all ice
                       ! 11 = mixture of ice and liquid
      ...
      integer :: k, n
      integer :: sd_fmnum_adj  ! randomly adujusted SD number that will be injected at this time step 
      integer :: cnt
      integer :: iexced       ! temporary
      integer :: ilist(1:sd_fmnum)
    ...
    !--------------------------------------------------------------------- !---------------------------------------------------------------------
      ...
      ! Randomly determine the SD number injected at this time step 
      tmp_rnd = gen_rand( sd_rng )
      sd_fmnum_adj = sd_fmnum-1
      if( tmp_rnd < (sd_fmnumreal-real(sd_fmnum_adj,kind=RP)) ) then
         sd_fmnum_adj = sd_fmnum_adj+1
      end if

      if(sd_fmnum_adj==0) return
      ...
      do n=1,sd_num
         ...
         if(cnt==sd_fmnum_adj) exit
      end do
      if(cnt<sd_fmnum_adj) then
         if( IO_L ) write(IO_FID_LOG,*) 'ERROR: Number of SDs not sufficinet for aerosol formation'
         if( IO_L ) write(IO_FID_LOG,*) 'Num of SDs needed:',sd_fmnum_adj,', Remaining SDs: ', cnt
         call PRC_MPIstop
      end if

      ! Initialized all super-droplets as water droplet
      !### status(liquid/ice) of super-droplets ###!
      sd_fmliqice(1:sd_fmnum_adj) = STAT_LIQ

      ! Set aerosol mass, freezing temperature, and muliplicity of super-droplets
      !! Get random number
      ...
      if( sdm_aslset==66 ) then
         ...
         do n=1,sd_fmnum_adj
            dry_r = 0.125e-6_RP ! [m]
            sd_fmasl(n,k) = F_THRD * ONE_PI                      &
                 * (dry_r*dry_r*dry_r) * rho_nacl
         
            sdn_tmp = (sdm_aslfmrate*sdm_aslfmdt*sdm_sdnmlvol)/real(sdm_aslfmsdnc,kind=RP)
         
            !! check multiplicity
            if( sdn_tmp<(2.0_RP**63.0_RP) ) then
               sd_fmn(n) = int(sdn_tmp,kind=DP)
               if( sd_fmx(n) < (sdn_tmp-real(sd_fmn(n),kind=RP)) )then
                  sd_fmn(n) = sd_fmn(n)+1
               end if
            else
               iexced = -1
         ...
         ! Initialized SDs freezing temperature
         if( sdm_cold ) then        
            !###### soluble aerosol ######!
            do n=1,sd_fmnum_adj
               sd_fmi%tf(n) = -38.d0  ! homogeneous freezing limit [degC]
            end do
         end if
      else
         if( IO_L ) write(IO_FID_LOG,*) 'sdm_aslset /= 66 is not supported for now'
         call PRC_MPIstop
      end if

!!$      !### Ammonium Sulfate [(NH4)2SO4] ###!
!!$      call sdm_aslsulf(sdm_aslfmrate,sdm_aslfmdt,                &
!!$                       sd_fmnum_adj,sd_numasl,sd_fmn,sd_fmasl,sd_fmnc)
      
      if( iexced<0 ) then
         if( IO_L ) write(IO_FID_LOG,*) "sdm_aslform, multiplicity exceeded the upper limit"
         call PRC_MPIstop
      end if
      ...
      do n=1,sd_fmnum_adj
         ...
         sd_fmx(n) = xmax_sdm * sd_fmx(n)+GRID_FX(IS-1)
         sd_fmy(n) = ymax_sdm * sd_fmy(n)+GRID_FY(JS-1)
         
         if( sd_fmn(n)>0 ) then !!! check muliplicity
      ...
      !### index[k/real] of super-droplets ###!
      call sdm_z2rk(sdm_zlower,sdm_zupper,       &
                        sd_fmnum_adj,sd_fmx,sd_fmy,sd_fmz,sd_fmri,sd_fmrj,sd_fmrk)

      ! Set droplet radius of super-droplets
      do n=1,sd_fmnum_adj
         sd_fmr(n) = 1.0E-15_RP
      end do

      !! below is commented out to inject dry NaCl
!!$      if( sdm_calvar(1) ) then
!!$
!!$         call sdm_rhot_qtrc2p_t(RHOT,QTRC,DENS,pres_scale,t_scale)
!!$
...
!!$                          sdm_aslmw,sdm_aslion,sdm_dtevl,           &
!!$                          pres_scale,t_scale,QTRC(:,:,:,I_QV),      &
!!$                          sd_fmnum_adj,sd_numasl,sd_fmliqice,sd_fmx,sd_fmy,sd_fmr,      &
...
!!$      end if
      ...
      if( sdm_cold .and. sdm_calvar(4) ) then
         ...
         call sdm_meltfreeze(                              &
              t_scale,pres_scale,QTRC(:,:,:,I_QV),         &
              sd_fmnum_adj,sd_fmliqice,sd_fmx,sd_fmy,      &
              sd_fmr,sd_fmri,sd_fmrj,sd_fmrk,sd_fmi)
     ...
     ! Set terminal velocity of super-droplets
      ...
      call sdm_getvz_liq(pres_scale,DENS,t_scale,                         &
                     sd_fmnum_adj,sd_fmliqice,sd_fmx,sd_fmy,sd_fmri,sd_fmrj,sd_fmrk,sd_fmr,sd_fmvz,   &
                     sd_itmp1(1:sd_fmnum_adj),sd_itmp2(1:sd_fmnum_adj),   &
                     sd_itmp3(1:sd_fmnum_adj),'no_interpolation')
      
      if( sdm_cold )then
         call sdm_getvz_ice(DENS,t_scale,            &
                           sd_fmnum_adj,sd_fmliqice,sd_fmx,sd_fmy,sd_fmri,sd_fmrj,sd_fmrk,sd_fmi,sd_fmvz,  &
                           sd_itmp1,'no_interpolation' )
       end if
    ...
    return
  end subroutine sdm_aslform1
  !----------------------------------------------------------------------------
  subroutine sdm_aslform2(DENS,RHOT,QTRC,                          &   
                         sdm_calvar,sdm_aslset,                   &
                         sdm_aslfmsdnc,sdm_sdnmlvol,              &
                         sdm_zupper,sdm_zlower,sdm_dtcmph,        &
                         sd_num,sd_numasl,sd_n,sd_liqice,sd_x,sd_y,sd_z,    &
    ...
    !--------------------------------------------------------------------- !---------------------------------------------------------------------
      ...
      ! Initialized all super-droplets as water droplets but freeze them later
      !### status(liquid/ice) of super-droplets ###!
      sd_fmliqice(1:sd_fmnum_adj) = STAT_LIQ
      ...
      if( sdm_aslset==66 ) then
         ...
         do n=1,sd_fmnum_adj
            ...
            sdn_tmp = (sdm_icefmrate*sdm_aslfmdt*sdm_sdnmlvol)/real(sdm_aslfmsdnc,kind=RP)
...
!!$                       sd_fmnum_adj,sd_numasl,sd_fmn,sd_fmasl,sd_fmnc)
      ...
      !! Below is commented out to inject dry NaCl (but this subroutine is for ice injection)
!!$      if( sdm_calvar(1) ) then
!!$
!!$         call sdm_rhot_qtrc2p_t(RHOT,QTRC,DENS,pres_scale,t_scale)
!!$
...
!!$      end if
      ...
      if( sdm_cold ) then
         ...
         do n=1,sd_fmnum_adj
            ...
            sd_fmliqice(n)  = STAT_ICE
            sd_fmi%re(n)    = 2.0d-6
            sd_fmi%rp(n)    = 2.0d-6
            sd_fmi%rho(n)   = dens_i_mks
            sd_fmi%mrime(n) = 0.0d0
            sd_fmi%nmono(n) = 1
            sd_fmr(n)       = 0.0d0
