...
!         REAL_CZ
    ...
    integer,intent(out) :: sdfmnum_s2c   ! Number of SDs added when the aerosol formation func is called. sdfmnum_s2c=int(sdfmnumreal_s2c)+1
    real(RP),intent(out) :: sdfmnumreal_s2c ! Number of SDs added when the aerosol formation func is called
...
!             zph_crs(k,i,j) = REAL_CZ(k,i,j)
    ...
    if( abs(sdm_aslset)>10 ) then
       ...
       sdfmnumreal_s2c = dtmp
       sdfmnum_s2c = int(dtmp)+1
    else
       sdfmnum_s2c = 1
    end if
...
!!$    sdnum_s2c = nint(sdininum_s2c) + bufsiz
    ...
    if( sdm_aslset == 66 ) then
       !### init+rest : NaCl (ICMW_PI) ###!
       sdnumasl_s2c = 1
    else if( abs(mod(sdm_aslset,10))==1 ) then
       !### init+rest : (NH4)2SO4 ###!
       sdnumasl_s2c = 1
