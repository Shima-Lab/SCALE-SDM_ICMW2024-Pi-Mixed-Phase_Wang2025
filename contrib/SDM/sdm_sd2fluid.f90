...
module m_sdm_sd2fluid
  ...
  implicit none
  private
  public :: sdm_sd2prec, sdm_sd2rhow, sdm_sd2rhocr, sdm_sd2qcqr, sdm_sd2qiqsqg, sdm_sd2rhosol, sdm_sd2rhosd, sdm_sd2rhodropmom, sdm_sd2rhoicemom
...
contains
  subroutine sdm_sd2prec(dtb_crs,                        &
    ...
    if( tlist_r>0 ) then
       ...
       write(basename_sd_out,'(A22,I6.6)') 'virtual_disdro.pe',mype


       inquire(file=trim(basename_sd_out), exist=exist)
       if (exist) then
        open(fid_sdm_o, file=trim(basename_sd_out), status="old", position="append", &
                        action="write", form="formatted", iostat=ierr)
       else
        open(fid_sdm_o, file=trim(basename_sd_out), status="new", position="append", &
                        action="write", form="formatted", iostat=ierr)
        write(fid_sdm_o, '(A)') ' time sd_x(n) sd_y(n) sd_r(n) sd_n(n) n '
       end if
       ...
       do m=1,tlist_r
         n=ilist_r(m)
          ...
          write(fid_sdm_o,'(4e16.8,i20,i10)')  TIME_NOWSEC, &
               &                      sd_x(n),           &
               &                      sd_y(n),           &
               &                      sd_r(n),           &
               &                      sd_n(n), n
    ...
    if( sdm_cold .and. (tlist_s>0) ) then
       ...
       write(basename_sd_out,'(A27,I6.6)') 'virtual_snow_disdro.pe',mype
       ...
       if (exist) then
         ...
         open(fid_sdm_o, file=trim(basename_sd_out), status="new", position="append", &
                         action="write", form="formatted", iostat=ierr)
         write(fid_sdm_o, '(A)') 'time sd_x(n) sd_y(n) sdi%re(n) sdi%rp(n) sdi%rho(n) sd_n(n), n'
       end if

       ! virtual disdrometer
       do m=1,tlist_s
             n=ilist_s(m)
           write(fid_sdm_o,'(6e16.8,i20,i10)') TIME_NOWSEC, &
                &                      sd_x(n),          &
                &                      sd_y(n),          &
                &                      sdi%re(n),        &
                &                      sdi%rp(n),        &
...
!!$    end do
  ...
  !-----------------------------------------------------------------------------
  subroutine sdm_sd2rhosd(zph_crs,rho_sdm,sd_num,sd_n,  &
       sd_x,sd_y,sd_r,sd_ri,sd_rj,sd_rk,sd_rkl,sd_rku,      &
       ilist)
    use scale_grid_index, only: &
         IA,JA,KA,IS,IE,JS,JE,KS,KE
    use m_sdm_common, only: &
         dxiv_sdm, dyiv_sdm, VALID2INVALID, num_threads
    use m_sdm_coordtrans, only: &
         sdm_x2ri, sdm_y2rj
    ! Input variables
    real(RP),intent(in) :: zph_crs(KA,IA,JA)    ! z physical coordinate
    ...
    do i=1,IA
    do i_threads=1,num_threads
       rho_sdm(k,i,j)=rho_sdm(k,i,j)+tmp_sdm(i_threads,k,i,j)
    end do
    end do
    end do
    end do
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  subroutine sdm_sd2rhodropmom(order_n,zph_crs,rho_sdm,sd_num,sd_n,sd_liqice, &
       sd_x,sd_y,sd_r,sd_ri,sd_rj,sd_rk,sd_rkl,sd_rku,      &
       ilist)
    use scale_grid_index, only: &
         IA,JA,KA,IS,IE,JS,JE,KS,KE
    use m_sdm_common, only: &
         dxiv_sdm, dyiv_sdm, VALID2INVALID, num_threads, STAT_LIQ, i2
    use m_sdm_coordtrans, only: &
         sdm_x2ri, sdm_y2rj
    ! Input variables
    ...
    if(tlist>0) then
       ...
       do m=i_threads,tlist,num_threads
          ...
          tmp_sdm(i_threads,k,i,j) = tmp_sdm(i_threads,k,i,j)                        &
               &                    + sd_r(n)**order_n * real(sd_n(n),kind=RP)
  ...
  !---------------------------------------------------------------------------------------------------------------------------------
  subroutine sdm_sd2rhoicemom(order_n,zph_crs,rho_sdm,sd_num,sd_n,sd_liqice, &
       sd_x,sd_y,sd_re,sd_rp,sd_ri,sd_rj,sd_rk,sd_rkl,sd_rku,      &
       ilist)
    use scale_grid_index, only: &
         IA,JA,KA,IS,IE,JS,JE,KS,KE
    use m_sdm_common, only: &
         !dxiv_sdm, dyiv_sdm, VALID2INVALID, num_threads, STAT_LIQ, i2
         dxiv_sdm, dyiv_sdm, VALID2INVALID, num_threads, STAT_ICE, i2
    use m_sdm_coordtrans, only: &
         sdm_x2ri, sdm_y2rj
    ! Input variables
    integer,intent(in)  :: order_n               ! order of the droplet moment
    ...
    !real(RP), intent(in) :: sd_r(1:sd_num) ! equivalent radius of super-droplets
    !add
    real(RP), intent(in) :: sd_re(1:sd_num) ! eqatorial radius of ice
    real(RP), intent(in) :: sd_rp(1:sd_num) ! polar radius of ice
    real(RP), intent(inout) :: sd_ri(1:sd_num)! index[i/real] of super-droplets
    real(RP), intent(inout) :: sd_rj(1:sd_num)! index[j/real] of super-droplets
    real(RP), intent(in) :: sd_rk(1:sd_num)! index[k/real] of super-droplets
    real(RP), intent(in) :: sd_rkl(IA,JA) ! lower boundary index[k/real] at scalar point in SDM calculation area
   ...
   ! Section specification for fapp profiler
   !edit
   call fapp_start("sdm_sd2rhoicemom",1,1)
#endif
   call sdm_x2ri(sd_num,sd_x,sd_ri,sd_rk)
   call sdm_y2rj(sd_num,sd_y,sd_rj,sd_rk)
   ...
   if(tlist>0) then
      ...
      do m=i_threads,tlist,num_threads
         ...
         tmp_sdm(i_threads,k,i,j) = tmp_sdm(i_threads,k,i,j)                        &
            &                    + max(sd_re(n), sd_rp(n))**order_n * real(sd_n(n),kind=RP)
...
#ifdef _FAPP_
   ! Section specification for fapp profiler
   call fapp_stop("sdm_sd2rhoicemom",1,1)
#endif
   return
  end subroutine sdm_sd2rhoicemom
!---------------------------------------------------------------------------------------------------------------------------------
