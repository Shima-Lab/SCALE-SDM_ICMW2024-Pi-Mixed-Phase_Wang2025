...
##### setup plot time
#plottimes <- alltimes
plottimes <- alltimes[which(as.numeric(alltimes)==5400)]
...
foreach(time = plottimes, .packages="plotrix") %dopar% {
	...
	#  if (time_count == 90){
	# 	stop("Process stopped at TIME = 90.")
	#  }
	 data <- NULL
	 # loop of MPI rank
	...
	# Probability Distribution Functions of drop & ice
	liqice_data <- valid_data[
  				  (valid_data$sd_liqice == 1  & valid_data$sd_r > 3.5e-6) |
         		  (valid_data$sd_liqice == 10 & pmax(valid_data$sdi_re, valid_data$sdi_rp) > 3.5e-6),
				  ]
	liqice_radi <- ifelse( liqice_data$sd_liqice == 1, liqice_data$sd_r, pmax(liqice_data$sdi_re, liqice_data$sdi_rp) )
	liqice_num  <- sum(liqice_data$sd_n)
	liqice_num_dens <- liqice_num/IMAX/JMAX/KMAX_sdm/length(allmpiranks)/DX/DY/DZ

    rstep <- 0.2
	pdf(paste("hist_PDF_drop_ice.",sprintf("%05d",time),".pdf",sep=""))
	if(length(liqice_radi)>1){
		hist_dsd <- weighted.hist(1.0e6*2*liqice_radi, liqice_data$sd_n/liqice_num/rstep,
                  breaks=seq(0,60,by=rstep),
                  main=paste("Probability Distribution Functions (T=", sprintf("%05d",time)," [s])"),
		  ylab="PDF [/um]",
                  xlab="radius [um]")
	}else{
		hist_dsd <- weighted.hist(seq(0,60,by=rstep), rep(1,length(seq(0,60,by=rstep))),
                  breaks=seq(0,60,by=rstep),
                  main=paste("Probability Distribution Functions (T=", sprintf("%05d",time)," [s])"),
		  ylab="PDF [/um]",
                  xlab="radius [um]")
	...
	dsd  <- ncvar_def("PDF", "/um", list(rdim), fill, longname="Probability Distribution Functions")

	outfile <- nc_create(filename=paste("./LES_Nxxx_PDF_drop_ice_", sprintf("%05d",time), sep=""),
	...
	# # Probability Distribution Functions of drop
    # rstep <- 0.2
	...
	# if(length(drop_radi)>1){
	# 	hist_dsd <- weighted.hist(1.0e6*drop_radi,
	# 			  drop_data$sd_n/IMAX/JMAX/KMAX_sdm/length(allmpiranks)/DX/DY/DZ/1.0e6, # [/m^3]->[/cm^3]
    #               breaks=seq(0,50,by=rstep),
    #               main=paste("Droplet size distribution (T=", sprintf("%05d",time)," [s])"),
	# 	  ylab="DSD [/cm^3]",
    #               xlab="radius [um]")
	...
	# rdim <- ncdim_def("r", "um", head(hist_dsd$breaks+rstep/2, n=length(hist_dsd$counts)),
    #                   longname="radius", unlim=TRUE)
	# dsd  <- ncvar_def("DSD", "/cm^3", list(rdim), fill, longname="Droplet size distribution")
	...
	# 	hist_isd <- weighted.hist(1.0e6*ice_radi,
	# 			  ice_data$sd_n/IMAX/JMAX/KMAX_sdm/length(allmpiranks)/DX*1.0e2/DY*1.0e2/DZ*1.0e2,
    #               breaks=seq(0,50,by=rstep),
    ...
    #                   longname="radius", unlim=TRUE)
	# isd  <- ncvar_def("ISD", "/cm^3", list(rdim), fill, longname="Ice size distribution")
...
# DSDvar    <- ncvar_def("DSD", "/um", list(rdim, tdim), fillvalue,
...
# Merge LES_Nxxx_PDF_drop_ice(by plottime) to single file
PDF_ALL <- NULL
isFirst <- TRUE
for(time in plottimes){
    ncfile <- paste("./LES_Nxxx_PDF_drop_ice_", sprintf("%05d",time), sep="")
    ncin <- nc_open(ncfile)
    if(isFirst){
        r <- ncvar_get(ncin,"r")
        ifFirst <- FALSE
    }
    PDF  <- ncvar_get(ncin,"PDF")
    PDF_ALL <- append(PDF_ALL, PDF)
    nc_close(ncin)
    file.remove(ncfile)
}
PDF_ALL <- array(PDF_ALL,dim=c(length(PDF), length(plottimes)))

rdim <- ncdim_def("r", "um", r)
tdim <- ncdim_def("t", "sec.", plottimes, unlim=TRUE)
fillvalue <- -9.9999e+30
PDFvar    <- ncvar_def("PDF", "/um", list(rdim, tdim), fillvalue,
            longname="Droplet size distribution")
outfile <- nc_create(filename="./LES_Nxxx_PDF_drop_ice", force_v4=TRUE, list(PDFvar))
ncvar_put(outfile, PDFvar, PDF_ALL)
ncatt_put(outfile, 0, "author", "Shima-Lab.")
nc_close(outfile)
